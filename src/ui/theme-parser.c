...
static GMarkupParser finestra_theme_parser = {
...
#define FINESTRA_THEME_FILENAME_FORMAT "metacity-theme-%d.xml"
            ...
            GError    **error)
{
  ...
  theme_filename = g_strdup_printf (FINESTRA_THEME_FILENAME_FORMAT, major_version);
  ...
  context = g_markup_parse_context_new (&finestra_theme_parser, 0, &info, NULL);
  ...
  for (version = THEME_VERSION; (version > 0); version--)
    {
      ...
      /* Look for themes in FINESTRA_DATADIR */
      theme_dir = g_build_filename (FINESTRA_DATADIR,
                                    "themes",
                                    theme_name,
                                    THEME_SUBDIR,
                                    NULL);
