...
{
  MetaScreen *screen;
  MetaWindow *window; /* May be NULL if this window isn't managed by Finestra */
  Window id;
         ...
         Window     xwindow)
{
  ...
  /* If Finestra has decided not to manage this window then the input events
                ...
                MetaWindow            *window)
{
  ...
  /* We are only interested in top level windows, others will
     be caught by normal finestra functions */
...
{
  compositor->show_redraw = (g_getenv ("FINESTRA_DEBUG_REDRAWS") != NULL);
  compositor->debug = (g_getenv ("FINESTRA_DEBUG_COMPOSITOR") != NULL);
                       ...
                       MetaWindow     *window)
{
   ...
   * enough about Finestra/X to know how else you are supposed to do it
   */
  meta_error_trap_push (xrc->display);
  switch (event->type)
    {
