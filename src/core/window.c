              ...
              int         state)
{
  ...
  /* Finestra doesn't use icon windows, so data[1] should be None
  ...
  if (!showing)
    {
       ...
       * test isn't duplicated here. Anyhow, we anigde2
      ...
      if (on_workspace && window->minimized && window->mapped &&
          !meta_prefs_get_reduced_resources ())
        {
          ...
          if (!result)
            {
              /* just anigde2 into the corner somehow - maybe
               * not a good idea...
               */
...
__window_is_terminal (MetaWindow *window)
{
  ...
  /* gde2-terminal -- if you couldn't guess */
  if (strcmp (window->res_class, "Gde2-terminal") == 0)
    return TRUE;
  /* xterm, rxvt, aterm */
  ...
  /* KTerm -- some terminal not KDE based; so not like Konsole */
    ...
    return TRUE;
  /* Multi-gde2-terminal */
  else if (strcmp (window->res_class, "Multi-gde2-terminal") == 0)
    return TRUE;
  ...
  if (!window->placed)
    {
       ...
       * still get placed when they are ultigde2ly shown.
       */
      window->placed = TRUE;
 ...
 * is useful for Finestra, but Finestra shouldn't need to specify a client
 * type for itself.  ;-)
 */
void
                                  ...
                                  int                  h)
{
   ...
   * are moved by finestra without resizing; send a configure notify
   * in such cases.  See #322840.  (Note that window->constructing is
   * only true iff this call is due to a MapRequest, and when
   * PPosition/UPosition hints aren't set, finestra seems to send a
                          ...
                          XEvent     *event)
{
   ...
   * and finestra's "pretend" focus window, and go through all
...
#define MAX_N_ACTIONS 12
  ...
  /* sticky according to EWMH is different from finestra's sticky;
   * finestra doesn't support EWMH sticky
   */
