 ...
 * kick everything off and then close down Finestra when it's time to go.
 */

/**
 * \mainpage
 * Finestra - a boring window manager for the adult in you
 *
 * Many window managers are like Marshmallow Froot Loops; Finestra
 * is like Cheerios.
 *
...
/**
 * Handle on the main loop, so that we have an easy way of shutting Finestra
 * down.
 */
static GMainLoop *meta_main_loop = NULL;

/**
 * If set, Finestra will spawn an identical copy of itself immediately
 * before quitting.
 */
...
/**
 * Prints log messages. If Finestra was compiled with backtrace support,
...
/**
 * Prints the version notice. This is shown when Finestra is called
 ...
 * build this copy of Finestra. This is actually always called
 ...
 * copy of Finestra. This is actually always called
 * on startup, but it's all no-op unless we're in verbose mode
 * (see meta_set_verbose).
 */
static void
meta_print_self_identity (void)
{
  ...
  meta_verbose ("Finestra version %s running on %s\n",
    VERSION, buf);
...
/**
 * The set of possible options that can be set on Finestra's
 ...
 * arguments that Finestra understands in meta_args.
 *
 * The strange call signature has to be written like it is so
 * that g_option_context_parse() gets a chance to modify argc and
 * argv.
 *
 * \param argc  Pointer to the number of arguments Finestra was given
 * \param argv  Pointer to the array of arguments Finestra was given
                    ...
                    MetaArguments *meta_args)
{
  ...
  GOptionEntry options[] = {
    ...
    },
    {
      ...
      &my_args.replace_wm,
      N_("Replace the running window manager with Finestra"),
      NULL
    },
    {
  ...
  g_option_context_add_main_entries (ctx, options, "finestra");
...
/**
 * Selects which display Finestra should use. It first tries to use
 * display_name as the display. If display_name is NULL then
 * try to use the environment variable FINESTRA_DISPLAY. If that
...
meta_select_display (gchar *display_name)
{
  ...
  else if (g_getenv ("FINESTRA_DISPLAY"))
    envVar = g_strconcat ("DISPLAY=",
      g_getenv ("FINESTRA_DISPLAY"), NULL);
...
main (int argc, char **argv)
{
  ...
  if (g_getenv ("FINESTRA_VERBOSE"))
    meta_set_verbose (TRUE);
  if (g_getenv ("FINESTRA_DEBUG"))
  ...
  bindtextdomain (GETTEXT_PACKAGE, FINESTRA_LOCALEDIR);
  ...
  meta_set_syncing (meta_args.sync || (g_getenv ("FINESTRA_SYNC") != NULL));
  ...
  if (g_getenv ("FINESTRA_G_FATAL_WARNINGS") != NULL)
  ...
  if (!meta_ui_have_a_theme ())
    {
      ...
      if (!(themes_dir = g_dir_open (FINESTRA_DATADIR"/themes", 0, &err)))
        {
...
/**
 * Stops Finestra. This tells the event loop to stop processing; it is rather
...
/**
 * Restarts Finestra. In practice, this tells the event loop to stop
 * processing, having first set the meta_restart_after_quit flag which
 * tells Finestra to spawn an identical copy of itself before quitting.
 * This happens on receipt of a _FINESTRA_RESTART_MESSAGE client event.
 */
void
meta_restart (void)
{
