...
ensure_logfile (void)
{
  if (logfile == NULL && g_getenv ("FINESTRA_USE_LOGFILE"))
    {
      char *filename = NULL;
      char *tmpl;
      int fd;
      ...
      tmpl = g_strdup_printf ("finestra-%d-debug-log-XXXXXX",
                              (int) getpid ());
...
meta_set_verbose (gboolean setting)
{
  ...
  if (setting)
    meta_fatal (_("Finestra was compiled without support for verbose mode\n"));
#else
  if (setting)
    ensure_logfile ();
#endif
