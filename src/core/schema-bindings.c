 ...
 * the Gde2Conf .schemas file.
 *
 * FIXME: also need to make 50-finestra-desktop-key.xml
 */
               ...
               const char *description)
{
  ...
  fprintf (target_file, "      <key>/schemas/apps/finestra/%s_keybindings/%s</key>\n",
            keybinding_type, name);
  fprintf (target_file, "      <applyto>/apps/finestra/%s_keybindings/%s</applyto>\n",
            keybinding_type, name);
  fprintf (target_file, "      <owner>finestra</owner>\n");
...
main (int argc, char **argv)
{
   ...
   * are hardcoded (in gtk/gtkaccelgroup.c; it's not finestra's fault).
