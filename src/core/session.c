 ...
 * libgde2ui, (C) Tom Tromey, Carsten Schaar)
...
/* We call any handler installed before (or after) gde2_ice_init but
                   ...
                   const char *previous_save_file)
{
...

  {
    ...
    prop1.num_vals = 1;
    prop1.vals = &prop1val;
    prop1val.value = "finestra";
    prop1val.length = strlen ("finestra");
...
#if 0 /* I think the Gde2Client rationale for this doesn't apply */
...
save_state (void)
{
   ...
   */
  finestra_dir = g_strconcat (g_get_user_config_dir (),
                              G_DIR_SEPARATOR_S "finestra",
                              NULL);

  session_dir = g_strconcat (finestra_dir,
                             G_DIR_SEPARATOR_S "sessions",
                             NULL);

  if (mkdir (finestra_dir, 0700) < 0 &&
      errno != EEXIST)
    {
      meta_warning (_("Could not create directory '%s': %s\n"),
                    finestra_dir, g_strerror (errno));
    }
  ...
  fprintf (outfile, "<finestra_session id=\"%s\">\n",
           client_id);
...
static GMarkupParser finestra_session_parser = {
...
load_state (const char *previous_save_file)
{
  ...
  if (!g_file_get_contents (session_file,
                            ...
                            &error))
    {
      ...
      /* Maybe they were doing it the old way, with ~/.finestra */
  ...
  context = g_markup_parse_context_new (&finestra_session_parser,
                                        0, &parse_data, NULL);
                        ...
                        GError             **error)
{
  ...
  if (strcmp (element_name, "finestra_session") == 0)
    {
      /* Get previous ID */
      int i;
          ...
          if (pd->previous_id)
            {
              g_set_error (error,
                       ...
                       G_MARKUP_ERROR_PARSE,
                           _("<finestra_session> attribute seen but we already have the session ID"));
              return;
            }
...
get_possible_matches (MetaWindow *window)
{
  ...
  ignore_client_id = g_getenv ("FINESTRA_DEBUG_SM") != NULL;
