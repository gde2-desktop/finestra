...
#undef item
  ...
  the_display->static_gravity_works = g_getenv ("FINESTRA_USE_STATIC_GRAVITY") != NULL;
...
#undef item
   ...
   */
  {
    ...
    meta_prop_set_utf8_string_hint (the_display,
                                    the_display->leader_window,
                                    the_display->atom__FINESTRA_VERSION,
                                    VERSION);
 ...
 *
 * https://github.com/stefano-k/Gde2-Desktop-Environment/commit/b0e5fb03eb21dae8f02692f11ef391bfc5ccba33
 */
 ...
 * it is the nexus, it is the Grand Central Station of Finestra's world.
    ...
    case ButtonPress:
      if ((window &&
      ...
      else if (window && display->grab_op == META_GRAB_OP_NONE)
        {
          ...
          if (g_getenv ("FINESTRA_DEBUG_BUTTON_GRABS"))
          ...
          if (event->type == FocusIn &&
              ...
              event->xfocus.detail == NotifyInferior)
            {
              meta_topic (META_DEBUG_FOCUS,
                          ...
                          "gde2-session logout dialog usage (see bug "
    ...
    case PropertyNotify:
      {
	     ...
	     */
	    if (event->xproperty.atom ==
		display->atom__FINESTRA_SENTINEL)
	      {
                  ...
                  if (showing_desktop)
                  ...
                  else
                    {
                      ...
                      meta_workspace_focus_default_window (screen->active_workspace, NULL, timestamp);
                       ...
                       display->atom__FINESTRA_RESTART_MESSAGE)
                {
                  meta_verbose ("Received restart request\n");
                  meta_restart ();
                }
              else if (event->xclient.message_type ==
                       display->atom__FINESTRA_RELOAD_THEME_MESSAGE)
                {
                  ...
                  meta_display_retheme_all ();
                       ...
                       display->atom__FINESTRA_SET_KEYBINDINGS_MESSAGE)
                {
                  ...
                  meta_set_keybindings_disabled (!event->xclient.data.l[0]);
                       ...
                       display->atom__FINESTRA_TOGGLE_VERBOSE)
                {
  ...
  if (display->grab_window)
    {
      ...
      display->grab_wireframe_active =
        (meta_prefs_get_reduced_resources () && !meta_prefs_get_gde2_accessibility ())  &&
                                  ...
                                  Window       xwindow)
{
  ...
  if (display->window_grab_modifiers != 0)
    {
      gboolean debug = g_getenv ("FINESTRA_DEBUG_BUTTON_GRABS") != NULL;
      int i;
      for (i = 1; i < 4; i++)
        {
                                     ...
                                     Window       xwindow)
{
  ...
  debug = g_getenv ("FINESTRA_DEBUG_BUTTON_GRABS") != NULL;
  i = 1;
  while (i < 4)
    {
      meta_change_button_grab (display, xwindow,
 ...
 * have any way to do so while Finestra is running, so it's rather
 * pointless.
 */
gboolean
  ...
  if ((hints_set && hints->decorations) || !hints_set)
    {
      ...
      /* Return estigde2d frame extents for a normal window. */
